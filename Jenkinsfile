properties([
    ...
    [$class: 'GithubProjectProperty', displayName: '', projectUrlStr: 'https://github.com/ossimlabs/omar-superoverlay'],
  ...
  node(POD_LABEL){
    ...
    stage('Docker build') {
      container('docker') {
        withDockerRegistry(credentialsId: 'dockerCredentials', url: "https://${DOCKER_REGISTRY_DOWNLOAD_URL}") {
          sh """
            docker build --network=host -t "${DOCKER_REGISTRY_PUBLIC_UPLOAD_URL}"/omar-superoverlay-app:${BRANCH_NAME} ./docker
          """
        }
      }
    }
    ...
    stage('Docker push'){
      container('docker') {
        ...
        sh """
            docker push "${DOCKER_REGISTRY_PUBLIC_UPLOAD_URL}"/omar-superoverlay-app:${BRANCH_NAME}
        """
        }
      }
    }
